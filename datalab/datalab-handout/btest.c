...
static int run_tests(int report) {
	...
	if (!test_fname || strcmp(test_set[i].name,test_fname) == 0) {
	    ...
	    max_points += rating;
	    if (grade)
		printf(" %.0f\t%.0f\t%d\t%s\n", tpoints, (float)rating, terrors, test_set[i].name);
	    if (report)
		printf("Test %s score: %.0f/%.0f\n",
	...
	printf("Overall correctness score: %.0f/%.0f\n", points, max_points);

    return errors;
}

static void usage(char *cmd) {
    printf("Usage: %s [-v 0|1] [-hg] [-f <func name>] [-e <max errors>]\n", cmd);
    printf("  -e <n>    Limit number of errors to report for single function to n\n");
    printf("  -f <name> Check only the named function\n");
    printf("  -g        Print compact grading summary (implies -v 1 and -e 0)\n");
